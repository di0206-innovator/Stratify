...
async function verifySupabaseToken(token) {
    ...
    try {
        if (supabase) {
            // Verify token using Supabase client (authoritative, network call)
        ...
        // Fallback: decode the JWT locally without a network round-trip.
        // Supabase JWTs are standard JWTs — we can read the payload claims
        // even without verifying the signature (useful for read-only auth gates).
        const parts = token.split('.');
        if (parts.length === 3) {
            const payload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8'));
            const now = Math.floor(Date.now() / 1000);
            if (payload.sub && payload.exp && payload.exp > now) {
                console.info('[Auth] Using JWT decode fallback for user:', payload.sub);
                return {
                    id: payload.sub,
                    email: payload.email || '',
        ...
        // Last-resort JWT decode on error
        try {
            const parts = token.split('.');
            if (parts.length === 3) {
                const payload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8'));
