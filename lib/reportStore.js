...
class FileReportStore {
    constructor({ filePath, maxReports = 500 } = {}) {
        this.filePath = filePath || getWritableDataPath('reports.json');
...
function canAccessReport(report, userId) {
    if (userId === 'api-token') return true;
    if (!report.ownerId) return true; // Unowned reports are publicly readable and claimable
...
module.exports = { FileReportStore, MemoryReportStore, stripHeavyFields, canAccessReport };
