...
class AuthService {
    constructor({ store, config, logger } = {}) {
        ...
        this.syncedExternalUsers = new Set();
    }
    ...
    async register({ email, password, name, username, role }) {
        ...
        const isProd = this.config && this.config.nodeEnv === 'production';
        const isTest = this.config && this.config.nodeEnv === 'test';
        const user = {
            ...
            role: role || 'founder', // 'founder', 'vc', 'angel'
        ...
        await this.store.update((state) => {
            ...
            state.emailOutbox.push(createOutboxEmail({
                ...
                subject: 'Verify your Stratify email',
                token,
    ...
    async syncExternalUser(externalUser, provider = 'external') {
        if (!externalUser || !externalUser.id) return;
        if (this.syncedExternalUsers.has(externalUser.id)) return;

        const existing = await this.findUserById(externalUser.id);
        if (!existing) {
            await this.store.update((state) => {
                const alreadyExists = state.users.some(u => u.id === externalUser.id);
                if (!alreadyExists) {
                    state.users.push({
                        id: externalUser.id,
                        email: normalizeEmail(externalUser.email),
                        name: String(externalUser.name || '').trim(),
            ...
            this.logger.info(`Synced ${provider} user to local store`, { userId: externalUser.id });
        }
    ...
    async updateWorkspaceProfile(userId, profile) {
        await this.store.update((state) => {
            ...
            if (user) {
                user.workspaceProfile = profile;
                user.role = profile.role || 'founder';
                user.updatedAt = new Date().toISOString();
            }
        });
        return true;
