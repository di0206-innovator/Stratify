...
class AuthService {
    constructor({ store, config, logger } = {}) {
        ...
        this.syncedFirebaseUsers = new Set();
    }
    ...
    async register({ email, password, name, username }) {
        ...
        const user = {
            ...
            passwordHash,
            emailVerified: isProd ? true : false,
    ...
    async syncFirebaseUser(firebaseUser) {
        if (!firebaseUser || !firebaseUser.id) return;
        if (this.syncedFirebaseUsers.has(firebaseUser.id)) return;

        const existing = await this.findUserById(firebaseUser.id);
        if (!existing) {
            await this.store.update((state) => {
                const alreadyExists = state.users.some(u => u.id === firebaseUser.id);
                if (!alreadyExists) {
                    state.users.push({
                        id: firebaseUser.id,
            ...
            this.logger.info('Synced Firebase user to local store', { userId: firebaseUser.id });
        }
        this.syncedFirebaseUsers.add(firebaseUser.id);
    }
}
