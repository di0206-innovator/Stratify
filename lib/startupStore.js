...
class PgStartupStore {
        ...
        if (existing) {
            await query(
                ...
                [
                    ...
                    startup.traction, startup.needs, startup.techStack || startup.tech_stack, startup.score || 0,
    ...
    async listTimeline(startupId, { limit = 100, eventType } = {}) {
        let sql = 'SELECT * FROM timeline_events WHERE startup_id = $1';
    ...
    async createBrief(brief) {
        await query(
            `INSERT INTO briefs (id, startup_id, title, content)
             VALUES ($1, $2, $3, $4)`,
            [brief.id, brief.startupId, brief.title, brief.content]
        );
        return brief;
    }
...
class FileStartupStore {
    ...
    async listTimeline(startupId, { limit = 100, eventType } = {}) {
        const events = await this.readJson(this.timelinePath);
        let filtered = events.filter(e => e.startupId === startupId);
        if (eventType) {
    ...
    async createBrief(brief) {
        ...
        try {
            ...
            briefs.unshift({ ...brief, createdAt: new Date().toISOString() });
    ...
    async listBriefs(startupId) {
        const briefs = await this.readJson(this.briefsPath);
        return briefs.filter(b => b.startupId === startupId);
    }
