...
class PgAuthService {
     ...
     */
    async register({ email, password, name, username, role }) {
        ...
        await withTransaction(async (client) => {
            await client.query(
                ...
                [userId, normalizedEmail, String(actualName).trim(), passwordHash, !isProd, finalRole]
            );
        ...
        return sanitizeUser({ id: userId, email: normalizedEmail, name: String(actualName).trim(), emailVerified: !isProd, role: finalRole, createdAt: new Date().toISOString() });
    }
    ...
    async requestPasswordReset(email) {
        ...
        await withTransaction(async (client) => {
            ...
            await client.query(
                `INSERT INTO email_outbox (id, to_email, type, subject, token, expires_at)
                 VALUES ($1, $2, $3, $4, $5, $6)`,
                [crypto.randomUUID(), normalizedEmail, TOKEN_TYPES.PASSWORD_RESET, 'Reset your Stratify password', token, tokenRecord.expiresAt]
            );
        });
    }
    ...
    async syncExternalUser(externalUser, provider = 'external') {
        ...
        if (this.redis) {
            try {
                const redisKey = `synced_users:${provider}`;
                const added = await this.redis.sadd(redisKey, externalUser.id);
                if (added === 0) {
                    this.syncedExternalUsers.add(externalUser.id);
                    return;
                }
            } catch (e) {
                this.logger.warn('Failed to update Redis user sync set (non-fatal):', e.message);
            }
        }
    ...
    async update(mutator) {
        // Run as a transaction to ensure atomic DB updates
        return withTransaction(async (client) => {
            const { rows: userRows } = await client.query(`SELECT id FROM users`);
            const currentIds = userRows.map(r => r.id);

            const state = {
                users: userRows.map(r => ({ id: r.id })),
                sessions: [],
                tokens: [],
                emailOutbox: []
            };
            ...
            const nextIds = new Set(state.users.map(u => u.id));
            const deletedIds = currentIds.filter(id => !nextIds.has(id));

            if (deletedIds.length > 0) {
                // Delete users (triggers CASCADE deletions for sessions & tokens in DB)
                await client.query(`DELETE FROM users WHERE id = ANY($1::text[])`, [deletedIds]);

                // Evict deleted user sessions from Redis
                if (this.redis) {
                    try {
                        const { rows: sessionRows } = await client.query(
                            `SELECT token_hash FROM sessions WHERE user_id = ANY($1::text[])`,
                            [deletedIds]
                        );
                        for (const s of sessionRows) {
                            await this.redis.del(`${SESSION_KEY_PREFIX}${s.token_hash}`);
                        }
                    } catch (e) {
                        this.logger.warn('Failed to clear Redis sessions on batch deletion:', e.message);
                    }
                }
            }
    ...
    async updateWorkspaceProfile(userId, profile) {
        await query(
            `UPDATE users SET workspace_profile = $1, role = $2, updated_at = NOW() WHERE id = $3`,
            [JSON.stringify(profile), profile.role || 'founder', userId]
        );
        if (this.redis) {
            try {
                const { rows } = await query(`SELECT token_hash FROM sessions WHERE user_id = $1`, [userId]);
                for (const row of rows) {
                    await this.redis.del(`${SESSION_KEY_PREFIX}${row.token_hash}`);
                }
            } catch (e) {
                this.logger.warn('Failed to clear Redis session cache after profile update:', e.message);
            }
        }
        return true;
    }
    ...
    async _getSessionFromCache(tokenHash) {
        ...
        try {
            ...
            this.logger.warn('Redis session cache read failed (non-fatal):', e.message);
            return null;
        }
    }
...
function pgUserToRecord(row) {
    return {
        ...
        workspaceProfile: row.workspace_profile || null,
    };
}
...
function sanitizeUser(user) {
    return {
        ...
        workspaceProfile: user.workspaceProfile || null,
    };
}
