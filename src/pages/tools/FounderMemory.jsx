      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-8 animate-fade-in text-[#111]">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-gray-200/60 select-none">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 bg-[#C8E64A] text-black px-4 py-2 font-outfit font-bold text-xs uppercase tracking-wider rounded-lg shadow-sm hover:bg-[#B5D235] transition-all cursor-pointer border-0"
            >
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-white border border-gray-200 p-6 rounded-xl space-y-4 animate-slide-up shadow-sm">
            ...
            <div>
              <label className="block text-xs font-bold uppercase text-gray-500 mb-1.5 tracking-wide">Context / Why</label>
              <textarea 
                value={context}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold uppercase text-gray-500 mb-1.5 tracking-wide">Status</label>
                <select
                  value={status}
              ...
              {status !== 'active' && (
                <div className="animate-slide-up">
                  <label className="block text-xs font-bold uppercase text-gray-500 mb-1.5 tracking-wide">Outcome / Learning</label>
                  <input 
                    value={outcome}
              ...
              <div key={d.id} className="os-card bg-white p-5 hover:border-black transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {d.context && (
                      <p className="text-xs text-gray-500 mb-4 whitespace-pre-wrap leading-relaxed font-light font-inter">{d.context}</p>
                    )}
                    {d.outcome && (
                      <div className="bg-[#FAF9F6] border border-gray-200 rounded-xl p-4 flex gap-3 select-none">
                        <Target size={16} className="shrink-0 mt-0.5 text-gray-400" />
                        <div>
                          <p className="text-[9px] font-bold uppercase text-gray-400 mb-1 tracking-wide">Outcome & Learning</p>
                          <p className="text-xs font-semibold text-gray-700 leading-relaxed font-inter">{d.outcome}</p>
                        </div>
                      </div>
                    )}
                  </div>
                  <div className="text-[10px] font-bold font-mono text-gray-400 shrink-0 select-none">
