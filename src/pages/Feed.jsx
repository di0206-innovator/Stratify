...
export default function Feed({ user, founderProfile }) {
  ...
  const getPostTypeStyle = (type) => {
    switch (type) {
      case 'milestone':
        return 'bg-[#A3E635] text-black border-black';
      case 'launch':
        return 'bg-[#C084FC] text-black border-black';
      case 'update':
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 relative z-10">
      {/* Page Header */}
      <div className="neo-card bg-white text-black p-8 mb-8 border-[4px] border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] select-none relative overflow-hidden flex flex-col items-center text-center">
        {/* Floating background decorative shape */}
        <div className="absolute -top-12 -left-12 w-28 h-28 bg-[#FCD34D] border-[4px] border-black transform rotate-45 opacity-80 pointer-events-none hidden md:block"></div>
        <div className="absolute -bottom-10 -right-10 w-24 h-24 bg-[#3B82F6] border-[4px] border-black rounded-full opacity-80 pointer-events-none hidden md:block"></div>
        
        <div className="inline-flex items-center gap-2 bg-[#EF4444] text-white border-[3px] border-black px-4 py-1.5 font-outfit font-black text-xs uppercase tracking-wider mb-4 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transform -rotate-1">
      ...
      {founderProfile && founderProfile.role === 'founder' && (
        <form onSubmit={handlePostSubmit} className="neo-card bg-white border-[4px] border-black p-6 mb-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 bg-[#A3E635] border-2 border-black flex items-center justify-center font-black text-sm uppercase">
          ...
          <textarea
            ...
            rows={3}
            required
            className="w-full p-4 border-[3px] border-black font-outfit font-bold text-sm focus:outline-none focus:ring-0 focus:border-black mb-4 bg-gray-50 text-black"
          />
          ...
          {(postType === 'milestone' || postType === 'launch') && (
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full p-3 border-[3px] border-black font-outfit font-bold text-sm focus:outline-none focus:ring-0 focus:border-black bg-gray-50 text-black mb-3"
              />
              
              {/* Agent Verification Console */}
              {(verifying || verificationLogs.length > 0) && (
                <div className="border-[3px] border-black p-3 bg-black text-green-400 font-mono text-xs select-text space-y-1 rounded-none text-left mb-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className={`px-3 py-1.5 border-[2px] border-black font-black text-xs uppercase cursor-pointer transition-all ${
                  postType === 'milestone' ? 'bg-[#A3E635] text-black' : 'bg-white text-black hover:bg-gray-100'
                }`}
              >
                ★ Milestone (+15 Score)
              </button>
              <button
                ...
                className={`px-3 py-1.5 border-[2px] border-black font-black text-xs uppercase cursor-pointer transition-all ${
                  postType === 'launch' ? 'bg-[#C084FC] text-black' : 'bg-white text-black hover:bg-gray-100'
                }`}
              >
                🚀 Product Launch
              </button>
      ...
      {loading ? (
        ...
        <div className="neo-card bg-white border-[4px] border-black p-12 text-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
        ...
        <div className="space-y-6">
          {posts.map((post) => (
            <div key={post.id} className="neo-card bg-white border-[4px] border-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] relative hover:translate-y-[-2px] transition-transform">
              ...
              {post.metadata?.powUrl && (
                <div className="mb-4 flex items-center gap-2 select-none">
                  <span className="bg-[#A3E635] text-black border-2 border-black px-2 py-0.5 text-[9px] font-black uppercase tracking-wider flex items-center gap-1">
