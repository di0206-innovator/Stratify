...
export default function FounderMemory({ founderProfile }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="bg-white border-[3px] border-black p-6 shadow-neo-hard flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 bg-[#A3E635] border-[3px] border-black px-4 py-2 font-black text-sm uppercase hover:shadow-neo-button transition-all"
          >
            {showForm ? 'Cancel' : <><Plus size={16} strokeWidth={3} /> Log Decision</>}
          </button>
          <button
            onClick={fetchDecisions}
            className="bg-white border-[3px] border-black p-2 hover:shadow-neo-button transition-all"
          >
      ...
      {/* Form */}
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white border-[3px] border-black p-6 shadow-neo-hard space-y-4">
          <div>
            ...
            <input 
              ...
              className="w-full border-[3px] border-black p-2 font-bold text-sm focus:outline-none focus:border-[#C084FC]"
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full border-[3px] border-black p-2 font-medium text-sm min-h-[100px] focus:outline-none focus:border-[#C084FC]"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full border-[3px] border-black p-2 font-bold text-sm focus:outline-none"
              >
          ...
          <button type="submit" className="w-full bg-black text-white border-[3px] border-black p-3 font-black text-sm uppercase tracking-wider hover:bg-gray-800 transition-colors">
            Save to Memory
          </button>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {decisions.map(d => (
            <div key={d.id} className="bg-white border-[3px] border-black p-5 shadow-neo-hard hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {d.status === 'validated' && <span className="bg-[#A3E635] border-[2px] border-black px-2 py-0.5 text-[10px] font-black uppercase flex items-center gap-1"><CheckCircle size={10} /> Validated</span>}
