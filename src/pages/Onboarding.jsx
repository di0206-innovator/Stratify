...
export default function Onboarding({ founderProfile, setFounderProfile }) {
  ...
  const handleSubmit = async (e) => {
    ...
    confetti({ particleCount: 180, spread: 90, origin: { y: 0.5 }, colors: ['#3B82F6', '#9333EA', '#10B981', '#F59E0B'] });
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F4] flex flex-col">
      {/* Top Bar */}
      <div className="w-full border-b border-gray-200 bg-white px-6 py-3 flex items-center justify-between select-none">
        <div className="flex items-center gap-2">
          <div className="relative w-7 h-7 flex-shrink-0">
            <div className="absolute w-4 h-4 rounded-full bg-red-500 border border-black -top-0.5 -right-0.5" />
            <svg className="absolute w-5 h-5 bottom-0 left-0" viewBox="0 0 100 100">
              <polygon points="50,10 90,90 10,90" stroke="black" strokeWidth="12" fill="#FCD34D" />
            </svg>
          </div>
          <span className="font-black text-sm tracking-tight uppercase">Stratify</span>
        </div>
        <div className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wider">
          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-white text-[10px] font-black ${step >= 1 ? 'bg-blue-600' : 'bg-gray-300'}`}>1</div>
          <div className={`h-px w-12 ${step >= 2 ? 'bg-blue-600' : 'bg-gray-300'}`} />
          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-white text-[10px] font-black ${step >= 2 ? 'bg-blue-600' : 'bg-gray-300'}`}>2</div>
          <span className="ml-2 text-gray-400">Workspace Setup</span>
        </div>
      </div>
          ...
          {step === 1 && (
            <div>
              ...
              <div className="text-center mb-10">
                <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-200 text-blue-700 px-4 py-1.5 font-bold text-xs uppercase tracking-wider mb-5 rounded-full">
                  <Sparkles size={14} /> WELCOME TO STRATIFY
                </div>
                <h1 className="text-3xl sm:text-4xl font-black text-gray-900 tracking-tight mb-3 normal-case">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                {ROLES.map(({ id, icon: Icon, label, desc, ring, iconBg }) => (
                  <button
                    ...
                    className={`flex flex-col items-center p-6 border-2 rounded-xl text-center cursor-pointer transition-all duration-150 ${
                      role === id ? `${ring} ring-2 shadow-md` : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm'
                    }`}
                  >
                    <div className={`p-3 rounded-full mb-3 ${role === id ? iconBg : 'bg-gray-100 text-gray-500'}`}>
                      <Icon size={24} />
                    </div>
                    <span className="font-bold text-sm text-gray-900 mb-2">{label}</span>
              ...
              <div className="flex justify-center">
                <button
                  type="button"
                  onClick={() => setStep(2)}
                  className="flex items-center gap-2 px-8 py-3 bg-gray-900 text-white text-sm font-bold rounded-lg hover:bg-gray-700 transition-colors"
                >
            ...
            <div>
              ...
              <div className="mb-8">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="flex items-center gap-1.5 text-xs font-bold text-gray-500 hover:text-gray-900 transition-colors mb-4"
                >
                  <ArrowLeft size={14} /> Back to role selection
                </button>
                <div className="flex items-center gap-3 mb-2">
                  <div className={`p-2 rounded-lg ${selectedRole.iconBg}`}>
              ...
              <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 sm:p-8 space-y-6">
                ...
                {role === 'founder' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5 tracking-wide">Startup Stage</label>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5 tracking-wide">Current Strategic Goal</label>
                ...
                {role === 'vc' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5 tracking-wide">Preferred Investment Stage</label>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5 tracking-wide">Investment Thesis</label>
                ...
                {role === 'government' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5 tracking-wide">Program Types Available</label>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5 tracking-wide">Ecosystem Goals</label>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  <button type="button" onClick={() => setStep(1)} className="flex items-center gap-1.5 text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors">
                    <ArrowLeft size={14} /> Back
                  </button>
                  <button type="submit" className="flex items-center gap-2 px-8 py-3 bg-gray-900 text-white text-sm font-bold rounded-lg hover:bg-gray-700 transition-colors">
...
function Field({ label, name, value, onChange, placeholder, required = false }) {
  return (
    <div>
      <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5 tracking-wide">
