...
const ROLES = [
  {
    ...
    ring: 'ring-2 ring-black border-black bg-white',
    iconBg: 'bg-[#C8E64A]/20 text-black',
  },
  {
    id: 'vc',
    icon: Landmark,
...
export default function Onboarding({ founderProfile, setFounderProfile }) {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] flex flex-col font-inter">
      {/* Top Bar */}
      <div className="w-full border-b border-gray-200 bg-white px-6 py-3.5 flex items-center justify-between select-none">
        ...
        <div className="flex items-center gap-3 text-xs font-bold text-gray-400 uppercase tracking-wider font-outfit">
          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[#111] text-[10px] font-black ${step >= 1 ? 'bg-[#C8E64A]' : 'bg-gray-200'}`}>1</div>
          <div className={`h-0.5 w-8 ${step >= 2 ? 'bg-[#1A1A1A]' : 'bg-gray-200'}`} />
          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[#111] text-[10px] font-black ${step >= 2 ? 'bg-[#C8E64A]' : 'bg-gray-200'}`}>2</div>
          <span className="ml-2 text-gray-500 font-semibold">Workspace Setup</span>
        </div>
      </div>
          ...
          {step === 1 && (
            <div className="animate-slide-up">
              ...
              <div className="text-center mb-10">
                <div className="inline-flex items-center gap-2 bg-[#C8E64A]/15 border border-[#C8E64A]/30 text-[#111] px-4 py-1.5 font-bold text-xs uppercase tracking-wider mb-5 rounded-full font-outfit">
                  <Sparkles size={14} /> WELCOME TO STRATIFY
                </div>
                <h1 className="text-3xl sm:text-4xl font-outfit font-black text-gray-900 tracking-tight mb-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                {ROLES.map(({ id, icon: Icon, label, desc, ring, iconBg }) => (
                  ...
                  >
                    <div className={`p-3 rounded-xl mb-4 ${role === id ? iconBg : 'bg-gray-50 text-gray-400'}`}>
                      <Icon size={24} />
                    </div>
                    <span className="font-outfit font-bold text-sm text-gray-900 mb-2">{label}</span>
                    <p className="text-xs text-gray-500 leading-relaxed">{desc}</p>
                    {role === id && (
                      <div className="mt-4 flex items-center gap-1.5 text-xs font-semibold text-[#111] bg-[#C8E64A]/30 px-2.5 py-1 rounded-full">
            ...
            <div className="animate-slide-up">
              ...
              <div className="mb-8">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="flex items-center gap-1.5 text-xs font-bold text-gray-400 hover:text-gray-900 transition-colors mb-4"
                >
                  <ArrowLeft size={14} /> Back to role selection
                </button>
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-lg bg-[#C8E64A]/25 text-black">
                    <selectedRole.icon size={20} />
                  </div>
                  <div>
                    <h2 className="text-xl font-outfit font-bold text-gray-900 leading-tight">
              ...
              <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 sm:p-8 space-y-6">
                ...
                {role === 'founder' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5 tracking-wide">Startup Stage</label>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5 tracking-wide">Current Strategic Goal</label>
                ...
                {role === 'vc' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5 tracking-wide">Preferred Investment Stage</label>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5 tracking-wide">Investment Thesis</label>
                ...
                {role === 'institution' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5 tracking-wide">Program Types Available</label>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5 tracking-wide">Ecosystem Goals</label>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  <button type="button" onClick={() => setStep(1)} className="flex items-center gap-1.5 text-sm font-semibold text-gray-400 hover:text-gray-900 transition-colors">
