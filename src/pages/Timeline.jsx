...
const EVENT_CONFIG = {
 milestone: { icon: Award, color: 'bg-emerald-500', label: 'Milestone' },
 launch: { icon: Zap, color: 'bg-[#FB923C]', label: 'Launch' },
 decision: { icon: Target, color: 'bg-purple-600', label: 'Decision' },
 ...
 startup_created: { icon: Sparkles, color: 'bg-emerald-500', label: 'Created' },
};
...
export default function Timeline({ founderProfile, user, openAuthModal }) {
 ...
 className="flex items-center gap-2 bg-white px-4 py-2 font-black text-sm uppercase hover: transition-all"
 >
 <Filter size={14} /> {filterOptions.find(f => f.value === filter)?.label} <ChevronDown size={14} />
 </button>
 {showFilterMenu && (
 <div className="absolute right-0 top-full mt-1 bg-white -hard z-50 min-w-[180px]">
 {filterOptions.map(opt => (
 <button
 key={opt.value}
 onClick={() => { setFilter(opt.value); setShowFilterMenu(false); }}
 className={`block w-full text-left px-4 py-2 text-sm font-bold hover:bg-gray-100 ${filter === opt.value ? 'bg-emerald-500' : ''}`}
 >
 {opt.label}
 </button>
))}
 ...
 className="bg-emerald-500 p-2 hover: transition-all"
 >
 ...
 <div className="bg-purple-600 p-4 inline-block mb-4">
 ...
 <div
 key={event.id || idx}
 className="bg-white p-4 -hard hover:translate-x-[-2px] hover:translate-y-[-2px] hover: transition-all"
 >
 ...
 <span className="bg-emerald-500 border-[2px] border-black px-2 py-0.5 text-[10px] font-black">
